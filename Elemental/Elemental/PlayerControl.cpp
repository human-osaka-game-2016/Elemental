...
void Player_Control()
{
	...
	if (g_Key[UP] == PUSH && g_player.skyFlag == false)
	{
		...
		g_player.speed = JUMP_POWER;	//! 加速度にジャンプの限界値を代入
	}
	...
	{
		/*!
		これを抜くと上の判定がなくなので薄い足場みたいなのが実装できる
		...
		*/
		...
		g_player.speed += GRAVITY;			//! 周りにブロックがなければ加速度にGRAVITYの値をプラスしてどんどん上に動かす
		g_player.posY += g_player.speed;	//! プラスされた加速度をさらに主人公のY座標にプラスしていく
